...
try:
    with open('./raw_report.json', 'r') as raw_file:
        raw_report = json.load(raw_file)
    with open('./preprocessed_report.json', 'r') as preprocessed_file:
        preprocessed_report = json.load(preprocessed_file)
except Exception as e:
    st.error(f"Error loading classification reports: {e}")
    st.stop()
